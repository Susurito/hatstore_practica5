...
class Admin_Notices {
	...
	public function get_black_friday_notice() {
        ...
        if ( $time > 1765497600 || '1' === get_option( 'bf25-last' ) ) {
			return;
		}
		...
		$link = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/black-friday/#bfdeals', 'wp-dash', 'bf25-notification', 'bf25' );
	...
	private function get_promotion_message( $type = 'new' ) {
		...
		return array(
			'message' => __( '<b>LAST CHANCE – Save Up To $105 on Premium Addons Pro</b>.', 'premium-addons-for-elementor' ),
		...
		if( isset( $license_data['status'] ) && 'valid' === $license_data['status'] ) {
			...
			if( isset( $license_data['id'] ) && '4' !== $license_data['id'] ) {
				...
				array_unshift( $stories['posts'], array(
					...
					'link'  => Helper_Functions::get_campaign_link( 'https://premiumaddons.com/docs/upgrade-premium-addons-license/', 'wp-dash', 'bf25-dash-widget', 'bf25' ),
				) );
